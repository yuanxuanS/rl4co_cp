...
class TabuSearch:
    ...
    def find_neighborhood(self):
        ...
        在一个解上更改，每子route pair都交换任意一个client，储存交换的client id
        """
        ...
        for id_1 in range(self.D):
            for id_2 in range(id_1, self.D):
                ...
                samp_1 = np.random.randint(1,len(solution[id_1])-1)     # 随机采样两段route
    ...
    def process_solution(self):
        ...
        for idx, path in enumerate(self.best_solution):
            ...
            self.processed_solution[self.drones[idx]] = temp_path       # give path to a drone
    ...
    def search(self, tabu_size=50, n_iters=1000):
        ...
        sol = self.generate_random_solution()       # nested list [[0, 2, 4,0], [], ...]
        self.initialize_solution(sol)  
        temp_move = None
        
        while n_iters > 0:
            nh, moves = self.find_neighborhood()
            for nb, move in zip(nh, moves): # 在领域内找一个不在禁忌表里面的move，作为best candidate
            ...
            for nb, move  in zip(nh[1:], moves[1:]):        # 和领域内其他解比较，找到更好的候选解
            ...
            if self.candidate_cost < self.best_cost:        # 领域内最好的解和之前的最好解比较
                self.best_solution = self.best_candidate
                self.best_cost = self.candidate_cost
            
            # Add candidate to tabu and update cost history
            self.TABU[temp_move] = tabu_size        # 候选解的move记录在禁忌表中，并用一个值记录在禁忌表中的优先排序
            self.costs.append(self.candidate_cost)  # 并记录下每一步的最优解
            self.best_costs.append(self.best_cost)
            
            # Remove moves from tabu list： 去掉记录值减为0的禁忌值
            moves_to_delete = []
