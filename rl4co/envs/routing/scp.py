...
class SCPEnv(RL4COEnvBase):
    """
    Set Cover Problem environment
    At each step, the agent chooses a city to add to set. 
    The reward is its cost.
    Terminate unless all the cities are covered.
    ...
    def _step(self, td: TensorDict) -> TensorDict:
        ...
        curr_covered_guidence_vec = SCPEnv.get_covered_guidence_vec(covered_node, curr_dist.clone())
        ...
        # The reward is cost of current node
        reward = -(td["costs"][range(td["costs"].size(0)), current_node] * ~done).unsqueeze(-1)      # [batch, ]
        reward += td["reward"]
    ...
    def _reset(self, td: Optional[TensorDict] = None, batch_size=None) -> TensorDict:
        ...
        init_costs = td["costs"] if td is not None else None
        ...
        if init_locs is None:
            ...
            init_locs, init_costs = data["locs"], data["costs"]
    ...
    def get_reward(self, td, actions) -> TensorDict:
        ...
        if td["reward"].dim() == 1:
            return td["reward"]
        elif td["reward"].dim() == 2:
            return td["reward"].squeeze()
    ...
    def generate_data(self, batch_size) -> TensorDict:
        ...
        return TensorDict({"locs": locs,
                           "costs": costs}, batch_size=batch_size)
